...
function getAuthCredentials() {
    return {
        auth_token: localStorage.getItem('railway_auth_token') || '',
        device_key: localStorage.getItem('railway_device_key') || ''
    };
}
...
function validateForm(event) {
    ...
    document.getElementById('auth_token').value = credentials.auth_token;
    document.getElementById('device_key').value = credentials.device_key;
    
    if (!credentials.auth_token || !credentials.device_key) {
    ...
    const validations = [
        ...
        { id: 'date', errorId: 'date-error', message: 'Date for Report is required' }
    ];
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const reportForm = document.getElementById("reportForm");
    if (reportForm) reportForm.addEventListener("submit", validateForm);
...
function showLoaderAndSubmit(event) {
    ...
    const searchIcon = submitButton.querySelector('.fa-file-pdf');
    ...
    if (searchIcon) {
        ...
        submitButton.innerHTML = loader.outerHTML + ' Generating Report...';
    }
...
function resetSubmitButton() {
    const submitButton = document.querySelector('#reportForm .btn-primary');
    if (submitButton) {
        ...
        const existingIcon = submitButton.querySelector('.fa-file-pdf');
        if (!existingIcon) {
            const pdfIcon = document.createElement('i');
            pdfIcon.className = 'fas fa-file-pdf';
            submitButton.prepend(pdfIcon);
        }
    }
}
...
async function searchTrainsBetweenStations() {
    ...
    try {
        ...
        if (!authCreds.auth_token || !authCreds.device_key) {
            showTrainSearchNetworkError('Please enter your Auth Token and Device Key first');
            return;
        }
        ...
        const response = await fetch('/search_trains', {
            ...
            body: JSON.stringify({
                ...
                auth_token: authCreds.auth_token,
                device_key: authCreds.device_key
            })
        });
